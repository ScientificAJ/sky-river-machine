  ...
  return {
    async queryTabs() {
      ...
      return tabs.flatMap((tab) => {
        const normalized = normalizeTab(tab, true);
    ...
    async getTab(tabId) { return normalizeTab(await browser.tabs.get(tabId), true); },
    async activateTab(tabId) { return normalizeTab(await browser.tabs.update(tabId, { active: true }), true)!; },
    async createTab(url) { return normalizeTab(await browser.tabs.create({ url }), true)!; },
    ...
    async discardTab(tabId) {
      ...
      return discarded ? normalizeTab(discarded, true) : null;
    },
